...
class ExcelRangePartitionRouter(SinglePartitionRouter):
    """
    Create ranges to request rows data to Microsoft Graph API for Excel.
    """
    ...
    def __init__(self, parameters: Mapping[str, Any]) -> None:
        ...
        self.worksheet_id = parameters.get("worksheet_id")
    ...
    def stream_slices(self) -> Iterable[StreamSlice]:
        ...
        while start_range <= self.sheet_row_count:
            end_range = min(start_range + self.batch_size - 1, self.sheet_row_count)
            logger.info(f"Fetching range A{start_range}:ZZ{end_range} for worksheet {self.worksheet_id}")
            yield StreamSlice(
                partition={"start_range": f"A{start_range}", "end_range": f"ZZ{end_range}"}, 
                cursor_slice={}
            )
...
class RawSchemaParser:
    ...
    def parse_raw_schema_values(
        ...
        """
        1. Parses worksheet headers from the provided raw schema. This method assumes that data is contiguous
        ...
        for property_index, raw_schema_property in enumerate(raw_schema_properties):
            ...
            if not raw_schema_property_value or (isinstance(raw_schema_property_value, str) and raw_schema_property_value.isspace()):
                break
...
@dataclass
class ExcelDataExtractor(DpathExtractor, RawSchemaParser):
    """
    Extract data from Excel API response format and match with headers.
    Microsoft Graph API returns Excel data in this format:
    {
      "values": [
    ...
    def extract_properties_to_match(self, properties_to_match, schema_type_identifier, names_conversion):
        if isinstance(properties_to_match, list) and properties_to_match:
            # Convert list of headers to indexed properties
            indexed_properties = {}
            for idx, header in enumerate(properties_to_match):
                if header and not (isinstance(header, str) and header.isspace()):
                    if names_conversion:
                        header = self._apply_name_conversion(header)
                    indexed_properties[idx] = header
            return indexed_properties
        return {}

    def _apply_name_conversion(self, header: str) -> str:
        """Apply name conversion based on config flags"""
        config = getattr(self, "config", {})
        flags = {
            "remove_leading_trailing_underscores": config.get("remove_leading_trailing_underscores", False),
            "combine_number_word_pairs": config.get("combine_number_word_pairs", False),
        ...
        if use_sanitzation:
            return safe_sanitzation_conversion(header, **flags)
        else:
            return safe_name_conversion(header)

    @staticmethod
    def match_properties_with_values(unmatched_values: List[Any], indexed_properties: Dict[int, str]):
        data = {}
        for relevant_index in sorted(indexed_properties.keys()):
            ...
            if unmatch_value is not None and str(unmatch_value).strip() != "":
                data[indexed_properties[relevant_index]] = str(unmatch_value)
        yield data

    @staticmethod
    def is_row_empty(cell_values: List[Any]) -> bool:
    ...
    @staticmethod
    def row_contains_relevant_data(cell_values: List[Any], relevant_indices: Iterable[int]) -> bool:
        for idx in relevant_indices:
            if len(cell_values) > idx and cell_values[idx] is not None and str(cell_values[idx]).strip() != "":
                return True
        return False
    ...
    def extract_records(self, response: requests.Response) -> Iterable[MutableMapping[Any, Any]]:
        ...
        for raw_record in raw_records_extracted:
            ...
            for unmatched_values in unmatched_values_collection:
                if not ExcelDataExtractor.is_row_empty(
                    unmatched_values
                ) and ExcelDataExtractor.row_contains_relevant_data(unmatched_values, self._indexed_properties_to_match.keys()):
                    yield from ExcelDataExtractor.match_properties_with_values(
                        unmatched_values, self._indexed_properties_to_match
                    )


@dataclass
class ExcelSchemaExtractor(DpathExtractor, RawSchemaParser):
    """
    Makes names conversion and parses worksheet headers from the provided row.
    """
...
def exception_description_by_status_code(code: int, workbook_id) -> str:
    if code in [
        ...
        return (
            "There was an issue with the Microsoft Graph API. "
            "This is usually a temporary issue from Microsoft's side. "
            "Please try again. If this issue persists, contact support."
        )
    if code == requests.status_codes.codes.FORBIDDEN:
        return (
            f"The authenticated user does not have permissions to view the workbook with id {workbook_id}. "
            "Please ensure the authenticated user has access to the Workbook and reauthenticate. "
            "If the issue persists, contact support."
        )
    if code == requests.status_codes.codes.NOT_FOUND:
        return (
            f"The requested Excel workbook with id {workbook_id} does not exist. "
            f"Please ensure the Workbook Link you have set is valid and the workbook exists. If the issue persists, contact support."
        )
